...
Use Forge's TypeScript/Webpack integration. The ESM worker and its production dependencies use a separate runtime.asar archive; native executables remain unpacked. This preserves Pi's assets and imports while preventing Squirrel/NuGet from processing 26,000 small files individually. The packaged smoke exercises actual Pi model/tool execution through a loopback-only synthetic provider. Development and runtime dependencies have separate checked-in lockfiles. Do not bundle a second LLM loop or rebuild upstream monorepos. Preserve dependency licenses in the archive and aggregate UI license texts.
...
Model tests are opt-in; default tests use a local synthetic provider. No actual health data are sent during build/test. Dependency vulnerability audit, package smoke, TypeScript checks and domain tests are separate evidence; warnings and failures are reported rather than concealed.
