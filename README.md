...
| 🧭 保留模糊信息并调整调查 | 记录原话、估计范围及未知项；调查计划随事实修订而更新。 |
| 📊 检查可用的健康依据 | 查看睡眠记录缺口、冲突，以及单一来源的心率、HRV、呼吸和血氧样本分布。 |
...
新增[本地插件、MCP 工具与睡眠调查 Skill](./plugins/sleepclaw/README.md)，与桌面端共用计算规则。插件直接使用宿主的模型，不需要额外配置 Pi 模型；首次从源码使用需构建插件，当前未发布到插件商店。健康原文件在本地处理，返回给宿主的摘要与事实可能进入宿主模型上下文。
